 ...
 *
 * Regra:
 *   Taxa = 3% do faturamento mensal do franqueado
 *   Mínimo: R$60 · Máximo: R$400
 *
 * Fluxo:
...
async function ensureCycle(franchiseeId: string, yearMonth: string) {
  ...
  return prisma.franchiseeBillingCycle.create({
    data: {
      franchiseeId,
      yearMonth,
      planPercent: user?.planPercent ?? 3, // default 3%
      status: "OPEN",
    },
  });
}
    ...
    if (customerId) {
      ...
      const pr = await fetch(`${BASE}/payments`, {
        ...
        body: JSON.stringify({
          ...
          value: amountPending,
          dueDate: due,
          description: `FireHub ${yearMonth} — Taxa de plataforma (3% · mín R$60 · máx R$400)`,
